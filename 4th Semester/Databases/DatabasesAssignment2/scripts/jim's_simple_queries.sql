...
/*
Find places that have reviews before 2011
Output: 167 rows
*/
SELECT listings.id, reviews.date
FROM listings INNER JOIN reviews 
ON listings.id=reviews.listing_id
WHERE reviews.date<'2011-01-01';



/*
Find all booked places between 2018-01-01 and 2018-01-10
Output: 55544 rows
*/
SELECT listings.id, listings.listing_url, calendar.date
FROM listings INNER JOIN calendar
ON listings.id=calendar.listing_id
WHERE calendar.date BETWEEN '2018-01-01' AND '2018-01-10' AND (calendar.available='f')
ORDER BY date;
