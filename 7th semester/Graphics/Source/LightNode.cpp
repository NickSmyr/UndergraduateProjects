...
LightNode::LightNode()
{
	...
	m_cast_shadow = false;
	// maybe this must be the same resolution as the origianl resolution
	m_shadow_map_width = 800;
	m_shadow_map_height = 600;
	m_shadow_map_bias = 0.001;
	m_shadow_map_texture = 0;
	m_shadow_map_fbo = 0;
	m_light_up_vector = glm::vec3(0, 1, 0);
}
...
void LightNode::CastShadow(bool cast)
{
	...
	if (cast)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24, m_shadow_map_width, m_shadow_map_height, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
...
void LightNode::SetPosition(const glm::vec3 & pos)
{
	...
	m_view_matrix = glm::lookAt(m_light_position, m_light_target, m_light_up_vector);
...
void LightNode::SetUpVector(const glm::vec3& vector)
{
	m_light_up_vector = vector;
}
...
void LightNode::SetShadowMapResolution(int width, int height) {
	m_shadow_map_width = width;
	m_shadow_map_height = height;
}
...
int LightNode::GetShadowMapHeight()
{
	return m_shadow_map_height;
}
int LightNode::GetShadowMapWidth()
{
