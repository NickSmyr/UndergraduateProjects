...
int main(int argc, char *argv[])
{
	...
	while (quit == false)
	{
		...
		float fps = 1 / dt;
		float target_fps = 70;
		float target_dt = 1 / target_fps;
		if (fps > target_fps) {
			int sleep_millis = (target_dt - dt)* 1000 ;
			std::this_thread::sleep_for(std::chrono::milliseconds(sleep_millis));
			// Calculate true dt
			// Compute the ellapsed time
			simulation_end = chrono::steady_clock::now();

			dt = chrono::duration <float>(
				simulation_end - simulation_start).count(); // in seconds
		}
		
		// TODo show fps , attainable fps every 1 sec
		
		if (cummulative_dt > 1) {
			//std::cout << "FPS are " << 1 / dt << " Attainable fps " << 1 / p_dt << std::endl;
			cummulative_dt -= 1;
		}
